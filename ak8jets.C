...
void LeptonsComp(){
  ...
  for( int evt = 0 ; evt < numEvents ; evt++ ){
    ...
    //cout << "ra2" << endl;
    vector<int> ra2b_indices_e = filterLeptons(ra2b_t->Electrons_fCoordinates_fPt);
    //cout << "nano" << endl;
    vector<int> nano_indices_e = filterLeptons(nano_t->Electron_pt);
      vector<int> ra2b_indices_m = filterLeptons(ra2b_t->Muons_fCoordinates_fPt);
      vector<int> nano_indices_m = filterLeptons(nano_t->Muon_pt);
      vector<int> ra2b_indices_p = filterLeptons(ra2b_T->Photons_fCoordinates_fPt);
      vector<int> nano_indices_p = filterLeptons(nano_t->Photon_pt);
    ...
    e_phi_ra2b[j]->Fill(ra2b_t->Electrons_fCoordinates_fPhi[ra2b_indices_e[j]]);
          mu_pt_ra2b[j]->Fill(ra2b_t->Muons_fCoordinates_fPt[ra2b_indices
                                                            ra2b_indices_m[j]]);
