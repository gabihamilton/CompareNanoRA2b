...
void electrons(){
    ...
    TH1F* iso_ra2b = new TH1F("TreeMaker iso",";Electron iso",50,0,35);
    iso_ra2b->SetLineWidth(2);
    iso_ra2b->SetLineStyle(1);
    iso_ra2b->SetLineColor(2);
    
    TH1F* iso_nano = new TH1F("Nano iso",";Electron iso",50,0,35);
    ...
    for( int evt = 0 ; evt < numEvents ; evt++ ){
        ...
        for(int j=0; j < nano_t->nElectron; j++ ){
            ...
            if (nano_t->Electron_pt[j]>10.0 && nano_t->Electron_cutBased[j]>0 && nano_t->Electron_dxy[j]<0.1 && nano_t->Electron_dz[j]<0.2 && (cut_bits & 0x37F) == 0x37F && nano_t->Electron_miniPFRelIso_all[j]<0.1){//except isolation: (cut_bits & 0x37F) == 0x37F){ //0:fail, 1:veto, 2:loose, 3:medium, 4:tight
                ...
                //iso_nano->Fill(nano_t->Electron_miniPFRelIso_all[j]*nano_t->Electron_pt[j]);
                iso_nano->Fill(nano_t->Electron_pfRelIso03_all[j]*nano_t->Electron_pt[j]);
